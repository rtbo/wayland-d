...
class EnumEntry
{
    ...
    {
        printCode(output, indent, camelName(name) ~ " = "~value~",");
    }
}
...
class Enum
{
    ...
    void printClientCode(File output, int indent)
    {
        printCode(output, indent, "enum "~dName~" : uint");
...
class Protocol
{
    ...
    void printHeader(File output, Options opt)
    {
        ...
        printDocComment(output, 0, format(
        ...
        printCode(output, 0, "module "~opt.moduleName~";");
